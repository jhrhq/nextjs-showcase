...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent className={cn("rounded-none overflow-hidden p-0", className)} showCloseButton={showCloseButton}>
        {children}
      </DialogContent>
    </Dialog>
  );
...
function CommandList({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.List>) {
  return (
    <CommandPrimitive.List
      ...
      className={cn("no-scrollbar max-h-72 scroll-py-0 outline-none overflow-x-hidden overflow-y-auto", className)}
      {...props}
    />
  );
}
