...
import { Globe, Plus, Sparkles, Trash2 } from "lucide-react";
...
function getDuplicateIndices(values: Array<{ url: string }>): Set<number> {
  ...
  seen.forEach((indices) => {
    if (indices.length > 1) {
      // Adding braces {} prevents implicit return
      indices.forEach((i) => {
        dupes.add(i);
      });
    }
...
function UrlStatsBadges({ total, valid, duplicates }: UrlStatsBadgesProps) {
  return (
    <div className="flex items-center gap-1.5 shrink-0">
      ...
      {valid > 0 && (
        <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 font-mono text-xs tabular-nums">
          {valid} valid
        </Badge>
      )}
      {duplicates > 0 && (
...
function DuplicateWarning({ count }: DuplicateWarningProps) {
  if (count === 0) return null;
  return (
    <>
      <Separator />
      <div className="px-6 py-3 bg-amber-50 flex items-start gap-2.5 rounded-b-xl">
...
//   meta: UrlFieldMeta;
//   errorMessage?: string;
//   control: FormValues extends infer T
//     ? T extends FormValues
//       ? import("react-hook-form").Control<T>
//       : never
//     : never;
...
// Use a simpler typing approach for the Control prop
type UrlFormControl = import("react-hook-form").Control<FormValues>;

interface UrlRowPropsClean {
  fieldId: string;
  index: number;
  total: number;
  meta: UrlFieldMeta;
...
function UrlRow({ fieldId, index, meta, total, errorMessage, control, onRemove, onPaste }: UrlRowPropsClean) {
  ...
  return (
    <Field data-invalid={meta.isInvalid ? true : undefined} className="gap-0">
      ...
      <InputGroup className={cn("transition-colors border", rowBorder)}>
        {/* Index + icon as inline-start addon */}
        ...
        {/* Status badges + remove button as inline-end addon */}
      ...
      {/* Per-field error rendered below the row */}
...
export default function UrlForm() {
  ...
  const handleBulkImport = useCallback(() => {
    ...
    if (nonEmpty.length === 0) {
      replace(
        newUrls.length > 0
          ? newUrls.map((url) => ({ url })) // map MUST return a value, so this is correct
          : [{ url: "" }]
      );
    } else {
      // Using block syntax ensures no value is returned to forEach,
      // regardless of what 'append' returns.
      newUrls.forEach((url) => {
        append({ url });
      });
    }
  ...
  // ── Single-field paste (multiple URLs detected) ────────────────────────────

  const onInputPaste = useCallback(
    (e: React.ClipboardEvent<HTMLInputElement>, idx: number) => {
      const text = e.clipboardData.getData("text");
      const urls = extractUrls(text);
      if (urls.length <= 1) return; // let RHF handle the normal single-URL paste
      e.preventDefault();
      ...
      const merged = [...watchedUrls.filter((_, i) => i !== idx && _.url.trim()), ...newUrls.map((url) => ({ url }))];
      ...
      <div className="min-h-screen bg-background flex items-start justify-center pt-12 pb-20 px-4">
        <div className="w-full max-w-2xl space-y-4">
          ...
          <div className="flex items-center gap-3 mb-2">
            <div className="size-9 rounded-lg bg-primary flex items-center justify-center" aria-hidden="true">
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
            ...
            <Card>
              ...
              <CardContent>
                {/*
                  FormFieldWrapper handles label, required marker,
                  InputGroup, error state, and FieldError automatically.
                */}
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  ...
                  <code className="text-xs bg-muted px-1 py-0.5 rounded">https://</code> URL and create fields
                  automatically.
                </CardDescription>
              </CardHeader>
              ...
              <CardContent className="space-y-3">
                {/*
                  FormFieldWrapper with as="textarea" renders InputGroupTextarea.
                  No label here — the CardDescription is sufficient context.
                  We attach the ref via the bottomAddon slot trick since the
                  textarea ref is managed internally; instead we use a plain
                  controlled ref on a sibling element for bulk read.
                */}
                <Field>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      Add, edit, or remove individual URLs. Duplicates are highlighted automatically.
                    </CardDescription>
                  </div>
