...
import type { UrlFieldStatus, UrlFormControl } from "./types";
import { ICON_COLOR_MAP, ROW_BORDER_MAP }       from "./constants";
...
export const UrlRow = React.memo(function UrlRow({
  ...
  return (
    <Field data-invalid={isInvalid ? true : undefined} className="gap-0">
      ...
      <InputGroup className={cn("transition-colors border", ROW_BORDER_MAP[status])}>
        ...
        <InputGroupAddon align="inline-start" className="flex items-center gap-2 pl-3 pr-1">
          ...
          <Link2
            className={cn("size-4 shrink-0 transition-colors", ICON_COLOR_MAP[status])}
            aria-hidden="true"
          />
        </InputGroupAddon>
