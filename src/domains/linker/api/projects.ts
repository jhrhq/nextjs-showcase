...
export const projectsApi = {
  getAll: async (): Promise<ProjectDTO[]> => {
    const response = await linkerApi.get("/linker/projects");
    return response.data.projects;
  },

  getById: async (id: string): Promise<ProjectDTO> => {
    const response = await linkerApi.get(`/linker/projects/${id}`);
    return response.data;
  },

  create: async (data: Partial<ProjectDTO>): Promise<ProjectDTO> => {
    const response = await linkerApi.post("/linker/projects", data);
    return response.data;
  },

  update: async (id: string, data: Partial<ProjectDTO>): Promise<ProjectDTO> => {
    const response = await linkerApi.put(`/linker/projects/${id}`, data);
    return response.data;
  },

  delete: async (id: string): Promise<void> => {
    await linkerApi.delete(`/linker/projects/${id}`);
  },

  getInboundLinks: async (projectId: string): Promise<InboundLink[]> => {
    const response = await linkerApi.get(`/linker/projects/${projectId}/inbound`);
    return response.data;
  },

  getSiloStructure: async (projectId: string): Promise<SiloStructure[]> => {
    const response = await linkerApi.get(`/linker/projects/${projectId}/silo`);
    return response.data;
  },

  getLinksReport: async (projectId: string): Promise<LinksReport[]> => {
    const response = await linkerApi.get(`/linker/projects/${projectId}/links-report`);
    return response.data;
  },

  getSiteReport: async (projectId: string): Promise<SiteReport> => {
    const response = await linkerApi.get(`/linker/projects/${projectId}/site-report`);
    return response.data;
  },
};
