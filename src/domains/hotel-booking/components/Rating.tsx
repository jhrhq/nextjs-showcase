...
export const Rating = forwardRef<HTMLInputElement, RatingProps>((props, ref) => {
  ...
  return (
    <div className="flex gap-2">
      <input {...props} className="hidden invisible" {...register(name)} ref={ref} />
      {Array(5)
        .fill(0)
        .map((_, i) => (
          <FaStar
            key={i}
            className={`text-2xl cursor-pointer ${i < +getValues(name) ? "text-yellow-500" : "text-gray-300"}`}
            onClick={() => setValue(name, i === +getValues(name) - 1 ? 0 : i + 1)}
          />
        ))}
    </div>
  );
